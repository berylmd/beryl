 ...
 */
export async function getFileContent(page: import('@playwright/test').Page, path: string): Promise<string | undefined> {
 ...
 */
export async function getWriteHistory(page: import('@playwright/test').Page): Promise<Array<{ path: string; content: string }>> {
 ...
 */
export async function setFileContent(page: import('@playwright/test').Page, path: string, content: string): Promise<void> {
  return page.evaluate(([path, content]) => (window as any).__BERYL_TEST_ADAPTER__?.setFile(path, content), [path, content])
}
